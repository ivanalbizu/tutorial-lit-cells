...
| `17-ecommerce` | [E-commerce](#17--e-commerce-con-lit--cells) | Catálogo, carrito, filtros, Custom Events, lazy loading |
...
Mini e-commerce funcional que integra todos los conceptos del tutorial en un proyecto realista: catálogo de productos, detalle, carrito de compras y navegación.
...
├── data/
│   └── products.ts          # Catálogo de productos (8 items)
├── pages/
│   ├── home/home-page.ts    # Catálogo con filtros por categoría
│   ├── product/product-page.ts  # Detalle con añadir al carrito
│   └── cart/cart-page.ts    # Carrito completo con cantidades
└── router/
    └── routes.ts            # 3 rutas: home, product/:id, cart
```

#### product-card — Componente reutilizable con Custom Events

El catálogo usa un componente `product-card` que se comunica con la página padre via eventos:
...
export class ProductCard extends LitElement {
  ...
  private _addToCart() {
    this.dispatchEvent(new CustomEvent('add-to-cart', {
      detail: { id: this.productId, name: this.name, price: this.price, image: this.image },
      bubbles: true,
      composed: true,
    }));
  }
...
La home-page escucha el evento y delega al `CartController`:
...
La home-page implementa filtros dinámicos usando `repeat()` y `@state`:
...
private get _filteredProducts(): Product[] {
  ...
  return PRODUCTS.filter(p => p.category.toLowerCase() === this._filter.toLowerCase());
}
```

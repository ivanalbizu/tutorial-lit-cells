...
speak "03-escena.mp3" "Creamos el componente product-page. El punto clave está en on Page Enter: se ejecuta cada vez que la página se vuelve activa. Dentro accedemos a la propiedad hash de la URL del navegador, que contiene la ruta completa incluyendo el parámetro, y lo parseamos para extraer el identificador."
...
speak "05b-escena.mp3" "La lógica es simple: si el destino es una ruta pública, devolvemos intercept false para dejar pasar. Si el usuario no está autenticado, devolvemos intercept true con un redirect al login. Si está autenticado, dejamos pasar."

speak "06-escena.mp3" "Aquí hay un error muy común. El valor de redirect NO es un string con el nombre de la página. Debe ser un objeto con la propiedad page. Si escribimos redirect login como string, Open Cells intentará hacer redirect.page y obtendremos andifaind. La navegación fallará silenciosamente."
...
speak "09-escena.mp3" "Para que el interceptor tenga datos que evaluar, usamos updateInterceptorContext importado desde open-cells-core. Cuando el usuario hace login correctamente, llamamos a esta función pasando un objeto con isAuthenticated true. Ese valor quedará disponible como ctx en el interceptor."

speak "09b-escena.mp3" "La página de login es sencilla: un botón que al pulsarse llama a updateInterceptorContext con isAuthenticated true y luego navega a la página protegida. La página protegida no necesita lógica especial, el guardia ya la protege en el interceptor."

speak "99-resumen.mp3" "Hemos cubierto tres capacidades clave del routing en Open Cells. Primero, rutas con parámetros dinámicos usando dos puntos en el path y parseando el hash en on Page Enter. Segundo, navegación con parámetros usando el segundo argumento de návikeit. Tercero, el interceptor como guardia de autenticación. Y hemos visto dos gotchas importantes: el redirect debe ser un objeto con la propiedad page, y skipNavigations no reemplaza al interceptor. En el próximo video veremos cómo compartir estado entre páginas usando los Channels de Open Cells."
